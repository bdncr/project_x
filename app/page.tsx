...
function Icon({ name }: { name: "search" | "plus" | "edit" | "trash" | "close" | "heart" | "eye" | "save" | "upload" | "user" | "logout" }) {
  const paths = {
...
export default function Home() {
  ...
  return <main>
    <header className="site-header"><a className="brand" href="#top"><span>Project</span><b>X</b></a><nav><a href="#explore">Судлах</a><a href="#share">Ажил нэмэх</a><a href="#manage">Миний ажил</a></nav>
      {user ? <div className="account-nav"><span><Icon name="user" /> {user.user_metadata.display_name || user.email?.split("@")[0]}</span><button onClick={signOut} aria-label="Гарах"><Icon name="logout" /></button></div> : <button className="primary-link" onClick={() => setAuthOpen(true)}><Icon name="user" /> Нэвтрэх</button>}
    </header>
    ...
    <section id="explore" className="explore-section"><div className="section-title"><p>Explore</p><h2>Бүтээлүүд</h2></div><div className="toolbar"><label className="search-box"><Icon name="search" /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Project X дээр хайх..." /></label><div className="filter-scroll">{categories.map((category) => <button key={category} className={activeCategory === category ? "filter active" : "filter"} onClick={() => setActiveCategory(category)}>{category}</button>)}</div><div className="sort-tabs"><button className={sortMode === "recommended" ? "active" : ""} onClick={() => setSortMode("recommended")}>Санал болгох</button><button className={sortMode === "recent" ? "active" : ""} onClick={() => setSortMode("recent")}>Шинэ</button><button className={sortMode === "liked" ? "active" : ""} onClick={() => setSortMode("liked")}>Талархал</button><button className={sortMode === "viewed" ? "active" : ""} onClick={() => setSortMode("viewed")}>Үзэлт</button></div></div>
